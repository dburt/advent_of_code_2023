...
digits = ('1'..'9').to_a + %w(one two three four five six seven eight nine)
digit_pattern = /(#{digits.join("|")})/
digit_map = [digits, ('1'..'9').to_a * 2].transpose.to_h
reverse_digit_pattern = /(#{digits.join("|").reverse})/

total = data.lines.map do |line|
  # n = line.scan(/\d/).values_at(0, -1).join.to_i  # 1st part
  first = line[digit_pattern, 1]
  last = line.reverse[reverse_digit_pattern, 1].reverse
  n = (digit_map[first] + digit_map[last]).to_i
  p [line, first, last, n]
  n
end.sum
